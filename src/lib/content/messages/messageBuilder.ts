...
import { formatSnoozeExpirationTimestamp, formatWatchExpirationTimestamp } from "../../helpers/datetime";
...
export function watchCommandResponseBuilder(watchData: WatchWithSnoozedWatches) {
    console.log('watchData = ', watchData)
    const price = watchData.priceRequirement ?? 'No Price Criteria'
    const formattedExpirationTimestamp = formatWatchExpirationTimestamp(watchData.created);
    const fields = [
        {
    ...
    if (watchData.snoozedWatches.length > 0) {
        const formattedSnoozeExpirationTimestamp = formatSnoozeExpirationTimestamp(watchData.snoozedWatches[0].endTimestamp)
        fields.push({
            name: "💤 💤 💤 💤  💤  💤 💤 💤 💤 💤  💤",
