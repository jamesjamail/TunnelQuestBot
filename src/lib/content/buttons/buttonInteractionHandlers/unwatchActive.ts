...
export default async function handleUnwatchActive(interaction: ButtonInteraction, metadata: Watch) {
    const data = await upsertWatch(interaction.user.id, metadata);
    console.log(data)
    const isSnoozed = !!data.snoozedWatches.length;
    const components = buttonRowBuilder(CommandTypes.watch, [isSnoozed, false, false]);
    const embeds = [watchCommandResponseBuilder(data)];
    await interaction.reply({ content: messageCopy.yourWatchIsActiveAgain, embeds, components });
}