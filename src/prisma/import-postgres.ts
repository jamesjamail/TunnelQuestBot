...
export async function importPostgres(
	...
	try {
		...
		const counts: Counts = {};
		for (const table of TABLES) {
			const columns = target.pragma(`table_info("${table}")`) as Column[];
			const names = columns
			...
			if (table !== 'User') {
				...
				const sequence = await source.query<{ value: string | null }>(
				...
				if (value !== null) {
					...
					if (!Number.isSafeInteger(highWater))
						throw new Error('Invalid source sequence');
		...
		if ((target.pragma('foreign_key_check') as unknown[]).length)
			throw new Error('Imported foreign keys are inconsistent');
