...
async function watches(message, args) {
	return await db.showWatch(message.author.id, args && args[0] ? args[0] : '', (res) => {
		if (res.success) {
...
async function list(message, args) {
	return await db.showWatches(message.author.id, (res) => {
		if (res.success) {
			const watches = [];
...
async function extend(message, args) {
	return await db.extendAllWatches(message.author.id);
	message.author.send('All watches succesfully extended for another 7 days.');
}

async function block(message, args) {
	if (args && args[1] && args[1] === 'BLUE' || args[1] === 'GREEN') {
		return await db.blockSeller(message.author.id, args[0], args[1], null);
		message.author.send(`Lets cut down the noise.  No longer notifying you about auctions from ${args[0]} on ${args[1]}.`);
	}
	else {
		return await db.blockSeller(message.author.id, args[0], null, null);
...
async function unblock(message, args) {
	if (args && args[1] && args[1] === 'BLUE' || args[1] === 'GREEN') {
		return await db.unblockSeller(message.author.id, args[0], args[1], null);
		message.author.send(`People change.  No longer blocking ${formatCapitalCase(args[0])} on ${formatCapitalCase(args[1])} server.`);
	}
	else {
		return await db.unblockSeller(message.author.id, args[0], null, null);
		message.author.send(`People change.  No longer blocking ${formatCapitalCase(args[0])} on either server.`);
	}
}

async function blocks(message, args) {
	return await db.showBlocks(message.author.id, (res) => {
		const blocks = [];
...
async function snooze(message, args) {
	// snooze all watches
	if (args && args[0]) {
		return await db.snooze('USER', message.author.id, args[0]);
		...
		return await db.snooze('USER', message.author.id);
...
async function unsnooze(message, args) {
	return await db.unsnooze('USER', message.author.id);
