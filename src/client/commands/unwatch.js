    ...
    .addSubcommandGroup((subcommandGroup) =>
      subcommandGroup
        ...
        .addSubcommand((subcommand) =>
          subcommand.setName("watches").setDescription("end all watches")
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("item")
            .setDescription("watch to remove")
            .setRequired(true)
        )
        .addStringOption((option) =>
          option
        ...
        return await unwatchAll(interaction)
          .then(async () => {
            return await interaction.reply({
              content: "All your watches have been removed.",
              ephemeral: true,
            });
          })
          .catch(async (err) => {
            ...
            if (res.rowCount === 0) {
              ...
              if (args.length > 1) {
                return interaction.reply({
                  content: `You don't have a watch for \`${item.toUpperCase()}\` on \`${server}\` server.`,
                  ephemeral: true,
                });
              }
              return interaction.reply({
                content: `You don't have a watch for \`${item.toUpperCase()}\`.`,
                ephemeral: true,
              });
            }
            return interaction.reply({
              content:
                "Your `" + item.toUpperCase() + "` watch has been removed.",
              ephemeral: true,
            });
          })
          .catch(async (err) => {
